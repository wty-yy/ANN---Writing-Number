...
const int NUM = 600; // Number of Learning Group (there must be GROUP * NUM = T)
const int TOT = 10; // Number of ANN, 10 30s, 1000 50min
...
void GroupLearn(volatile int *st, vi *perm, layer grad[], db *cost) { // Thread of Group Learning with start id
	while (1) {
		if ((*st) >= 0) { // get a start learning position
			for (int j = *st; j < *st + GROUP; j++) { // assign Learning tasks
				*cost += BP((*perm)[j], grad);
			}
			*st = -1; // standby
		}
		if ((*st) == -2) break; // close the thread
		//usleep(1);
	}
}
void ANN() { // Artificial Neural Network
	...
	thread th[THR];
	volatile int st[THR];
	for (int t = 0; t < THR; t++) { // initialize all thread
		st[t] = -1; // standby
		th[t] = thread(GroupLearn, &st[t], &perm, grad[t], &cost[t]);
	}
	for (int _i = 0; _i < TOT; _i++) {
		db Cost = 0;
		random_shuffle(perm.begin(), perm.end()); // Random for each Training Date
		for (int i = 0; i < GROUP * NUM; i += GROUP * THR) { // start position for all threads
			// initialization grad[] and cost[]
			for (int t = 0; t < THR; t++) {
				for (int l = 0; l < L; l++)
					grad[t][l] = layer(l);
				cost[t] = 0;
